...
struct DiscoverView: View {
            ...
            VStack(spacing: 20) {
                ...
                ZStack {
                    if viewModel.isLoading {
                        ...
                        VStack {
                            Text("you rotted through all the words")
                                .font(.title)
                                .foregroundColor(.black)
                        ...
                        VStack(alignment: .leading, spacing: 0) {
                            // Main content area with top band
                            ZStack(alignment: .top) {
                                ...
                                // Darker yellow top band exactly matching the card width
                                Rectangle()
                                    .fill(Color(hex: "D4B136")) // Darker yellow for the top edge
                                    .frame(height: 20) // Doubled the thickness
                        ...
                        // Added outline based on swipe direction
                        .overlay(
                            Rectangle()
    ...
    // Dynamic outline color based on swipe direction
    private var outlineColor: Color {
        if cardOffset > 20 { // Even slight right swipe shows green
            // Increase opacity as you swipe further
            let opacity = min(0.3 + abs(cardOffset) / 200, 1.0)
            return Color.green.opacity(opacity)
        } else if cardOffset < -20 { // Even slight left swipe shows red
            // Increase opacity as you swipe further
            let opacity = min(0.3 + abs(cardOffset) / 200, 1.0)
            return Color.red.opacity(opacity)
        } else {
            return Color.clear // No outline when card is centered
        }
    }
    
    // Dynamic outline width based on swipe distance
    private var outlineWidth: CGFloat {
        let baseWidth: CGFloat = 4.0
        let maxWidth: CGFloat = 12.0 // Increased maximum width
        
        if abs(cardOffset) < 20 {
            return 0 // No outline when barely moving
        } else {
            // More aggressive width scaling with swipe distance
            let scaleFactor = min(abs(cardOffset) / 50, 1.0) // Normalize swipe distance
            let width = baseWidth + scaleFactor * (maxWidth - baseWidth)
            return width
        }
    }
